...
%% Include path.
%%
%% The include path is where headers are expected to live. The order will be kept when
%% searching, so an include path containing ['/foo', '/bar'] where both /foo and /bar
%% contains a header called "foobar.h", /foo/foobar.h will be used.
